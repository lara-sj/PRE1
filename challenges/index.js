...
function getItemsLongerThan(array, len) {
  ...
  return array.filter((element) => element.length > len);
}
...
function flattenArrayByOne(arrayOfArrays) {
  ...
  // [[1],[2],[[3,4]]] => [1,2,[3,4]]
  const newArray = [];
  arrayOfArrays.map((array) => newArray.push(...array));
  return newArray;
}
...
function isItemOmnipresent(arrayOfArrays, item) {
  ...
  return (
    arrayOfArrays.filter((array) => array.includes(item)).length ==
...
function getNorthcodersNames(northcoders) {
  ...
  const names = []
  northcoders.map(coder => names.push(coder.name))
  return names
}
...
function countTheObjects(arr) {
  /*
    ...
    */
   let count = 0
   return arr.filter(element => element instanceof Object && element.length == undefined).length
}
